...
export function HomeSearchForm() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    // Route to browse for non-logged-in users, tenant properties for logged-in
    const basePath = user ? "/tenant/properties" : "/browse"
    if (query) {
  ...
  const selectSuggestion = (township: Township) => {
    ...
    setShowSuggestions(false)
    const basePath = user ? "/tenant/properties" : "/browse"
    router.push(`${basePath}?search=${encodeURIComponent(township.name)}`)
  }
