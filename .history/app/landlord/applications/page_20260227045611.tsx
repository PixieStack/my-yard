...
export default function LandlordApplicationsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // 2. Fetch applications + viewing_requests + tenants in parallel
      ...
      const combined: Application[] = applicationsData.map((app) => ({
        ...
        viewing_request:
          viewingRequestsData.find(
            (vr) => vr.property_id === app.property_id && vr.tenant_id === app.tenant_id
          ) ?? null,
      }))

      setApplications(combined)

      // 5. Standalone viewings = viewing requests with NO matching application yet
      ...
      const standalone: StandaloneViewing[] = viewingRequestsData
        .filter((vr) => !applicationTenantPropertyPairs.has(`${vr.property_id}__${vr.tenant_id}`))
        .map((vr) => ({
          ...vr,
  ...
  // ─── Confirm Viewing (Landlord accepts the time) ──────────────────────────
  ...
  const handleConfirmViewing = async () => {
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          status: "confirmed",
          landlord_message: confirmViewingNote.trim() || null,
        })
  ...
  // ─── Mark Viewing as Completed (viewing physically happened) ─────────────

  const handleMarkViewingDone = async (viewingId: string, applicationId: string | null) => {
    setConfirmingViewing(true)
    try {
      const { error } = await supabase
        .from("viewing_requests")
      ...
      // Optionally update application status so landlord knows viewing is done
      if (applicationId) {
        await supabase
          .from("applications")
  ...
  const handleDeclineViewing = async () => {
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          status: "declined",
          landlord_message: declineViewingNote.trim() || null,
        })
  ...
  // ─── Schedule Viewing (legacy from applications) ──────────────────────────
  ...
  const scheduleViewing = async () => {
    ...
    try {
      ...
      const { error: viewingError } = await supabase.from("viewing_requests").insert({
        ...
        status: "confirmed",
        landlord_message: viewingMessage || null,
      })
    ...
    try {
      ...
      if (status === "approved") {
        ...
        if (application) {
          ...
          // Deactivate property + reject other applications
          await supabase
            .from("properties")
  ...
  return (
    <div className="space-y-6">
                ...
                <CardContent className="pt-6">
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        {application.tenant?.phone && (
                          <p className="text-sm text-gray-600">{application.tenant.phone}</p>
                        )}
                      </div>
                    </div>
                  ...
                  {application.viewing_request && (
                    <div className="mb-4 p-3 bg-blue-50 border border-blue-100 rounded-lg">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <Badge className={getViewingStatusColor(application.viewing_request.status)}>
                      ...
                      {/* Confirm viewing buttons if status is pending/requested */}
                      ...
                      {/* Mark as Done button if confirmed */}
                  ...
                  {application.tenant_notes && (
                    <div className="mb-4 p-3 bg-gray-50 rounded-lg">
                      <p className="text-sm text-gray-500 mb-1">Message from applicant:</p>
                  ...
                  <div className="flex items-center justify-between border-t pt-4">
                    <p className="text-sm text-gray-500">
                      Applied {new Date(application.applied_at).toLocaleDateString("en-ZA")}
                    </p>
                      ...
                      {!["approved", "rejected"].includes(application.status) && (
                        <>
                          {application.status === "pending" &&
                            !application.viewing_request && (
                              <Button
                                ...
                                onClick={() =>
                                  setViewingDialog({ open: true, applicationId: application.id })
                                }
                              >
                          ...
                          <Button
                            ...
                            onClick={() =>
                              setRejectionDialog({ open: true, applicationId: application.id })
                            }
                          >
                      ...
                      <CardContent className="pt-4 pb-4">
                        <div className="flex items-start justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-gray-500">{viewing.tenant.email}</p>
                            </div>
                          </div>
                        ...
                        <div className="flex gap-2 mt-4">
                          ...
                          <Button
                            ...
                            className="text-red-600 border-red-200 hover:bg-red-50"
                            onClick={() =>
                              setDeclineViewingDialog({ open: true, viewingId: viewing.id })
                            }
                          >
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="p-3 bg-green-50 border border-green-200 rounded-lg text-sm text-green-800">
              ...
              The tenant will be able to submit a full application once the viewing is completed.
            </div>
      ...
      <Dialog
        ...
        onOpenChange={(open) => setDeclineViewingDialog({ open, viewingId: null })}
      >
        <DialogContent>
          <DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-2">
              <Button
                variant="outline"
                onClick={() => setDeclineViewingDialog({ open: false, viewingId: null })}
              >
                Cancel
              </Button>
              <Button
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-2">
              <Button
                variant="outline"
                onClick={() => setViewingDialog({ open: false, applicationId: null })}
              >
                Cancel
              </Button>
      ...
      >
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <p className="text-sm">Are you sure you want to approve this application?</p>
            <div className="flex justify-end gap-2">
              <Button
                variant="outline"
                onClick={() =>
      ...
      <Dialog
        ...
        onOpenChange={(open) => setRejectionDialog({ open, applicationId: null })}
      >
        <DialogContent>
          <DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-2">
              <Button
                variant="outline"
                onClick={() => setRejectionDialog({ open: false, applicationId: null })}
              >
                Cancel
              </Button>
              <Button
