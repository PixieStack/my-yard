...
import { notifyApplicationApproved, notifyApplicationRejected, notifyViewingConfirmed, notifyViewingCompleted } from "@/lib/notifications"
...
export default function LandlordApplicationsPage() {
  ...
  useRealtimeSubscription<ApplicationPayload>(
    "applications",
    (payload) => {
      console.log("📡 Landlord applications update:", payload)
      fetchData()
    },
  ...
  const handleConfirmViewing = async () => {
    ...
    try {
      ...
      const viewingData = await supabase
        .from("viewing_requests")
      ...
      if (viewingData.data) {
        const propertyData = await supabase
          .from("properties")
          .select("title")
          .eq("id", viewingData.data.property_id)
          .maybeSingle()

        if (propertyData.data) {
          try {
            await notifyViewingConfirmed(
              viewingData.data.tenant_id,
              propertyData.data.title,
              confirmViewingDialog.viewingId
            )
          } catch (notifyErr) {
