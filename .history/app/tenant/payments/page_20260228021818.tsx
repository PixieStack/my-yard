...
import { OzowPaymentService, OzowPaymentRequest } from "@/lib/ozow"
...
export default function TenantPaymentsPage() {
  ...
  const [processingOzowPayment, setProcessingOzowPayment] = useState(false)
  const [ozowLeaseId, setOzowLeaseId] = useState<string | null>(null)
  ...
  const initiateOzowPayment = async (leaseId: string) => {
    if (!profile?.id) return

    try {
      setProcessingOzowPayment(true)
      setOzowLeaseId(leaseId)
      ...
      // Create API call to initiate payment
      const response = await fetch('/api/payments/initiate-ozow', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          leaseId,
          tenantId: profile.id,
          amount: lease.move_in_total,
          description: `Move-in payment for ${lease.property_title}`,
        }),
      })
      ...
      setProcessingOzowPayment(false)
      setOzowLeaseId(null)
    }
  }
          ...
          <Button
            ...
            onClick={() => {
              ...
              if (activeLeases.length === 1) {
                initiateOzowPayment(activeLeases[0].id)
              } else {
                alert('Please select a lease from the list below')
              }
            }}
            disabled={processingOzowPayment}
            data-testid="pay-with-ozow-btn"
          >
            {processingOzowPayment ? (
