...
import { notifyAdminFeeRequired } from "@/lib/notifications-extended"
import {
...
export default function TenantLeasesPage() {
  ...
  const signLease = async () => {
    ...
    try {
      ...
      // If fully signed, create payment record and notify
      if (landlordSigned) {
        try {
          // Create move-in payment
          const moveInAmount = (config as any).move_in_total || signingLease.deposit_amount
          const { error: paymentError } = await supabase.from("payments").insert({
            lease_id: signingLease.id,
            tenant_id: profile?.id,
            landlord_id: signingLease.landlord_id,
            amount: moveInAmount,
            type: "move_in",
            status: "pending",
          ...
          // Notify landlord about admin fee
          try {
            const notifyAdminFeeRequired = require("@/lib/notifications-extended").notifyAdminFeeRequired
            await notifyAdminFeeRequired(signingLease.landlord_id, signingLease.property?.title || "Unknown Property")
          } catch (notifyErr) {
            console.error("Error sending admin fee notification:", notifyErr)
          }

          // Redirect to payment
          window.location.href = `/tenant/payments?lease_id=${signingLease.id}`
        } catch (err) {
