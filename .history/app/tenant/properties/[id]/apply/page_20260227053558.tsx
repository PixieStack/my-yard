...
function getBlockedMessage(viewing: ViewingRequest | null): string {
  if (!viewing) {
    return "You need to request a viewing before applying for this property."
  }
  switch (viewing.status) {
    case "pending":
    case "requested":
      return "Your viewing request is awaiting landlord confirmation. You can apply once confirmed."
    case "scheduled":
      return "Your viewing has been scheduled. You can apply once the landlord marks it as completed."
    case "declined":
      return "Your viewing request was declined. You cannot apply for this property."
    case "cancelled":
      return "Your viewing was cancelled. Please contact support if you believe this is an error."
    case "application_submitted":
      return "You have already submitted an application for this property."
    default:
      return "Your viewing must be confirmed before you can apply."
  }
}
...
export default function ApplyForPropertyPage() {
  ...
  // ── Form state using ACTUAL DB column names ────────────────────────────────
  const [proposedMoveIn, setProposedMoveIn] = useState("")           // → preferred_move_in_date
  const [leaseDuration, setLeaseDuration] = useState("12")           // → lease_duration_months
  const [additionalOccupants, setAdditionalOccupants] = useState("0") // → additional_occupants
  const [occupantDetails, setOccupantDetails] = useState("")          // → additional_occupants_details
  const [coverLetter, setCoverLetter] = useState("")                  // → cover_letter
  const [specialRequests, setSpecialRequests] = useState("")          // → special_requests
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const view = viewingResult.data as ViewingRequest | null
      ...
      // Gate check
      if (!view || !canApply(view.status)) {
        setPageError(getBlockedMessage(view))
      }
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (parseInt(leaseDuration) < (property.minimum_lease_months || 0)) {
      setError(`Minimum lease duration is ${property.minimum_lease_months} months.`)
      return
    }
    ...
    try {
      // ── Insert using ACTUAL column names from your DB schema ─────────────
      const insertPayload = {
        ...
        // ── Original DB columns ──────────────────────────────────────────
        preferred_move_in_date: proposedMoveIn,                    // ✅ original column
        lease_duration_months: parseInt(leaseDuration),            // ✅ original column
        cover_letter: coverLetter.trim() || null,                  // ✅ original column

        // ── Added columns (from migration) ───────────────────────────────
        additional_occupants: parseInt(additionalOccupants),       // ✅ added column
        additional_occupants_details: occupantDetails.trim() || null, // ✅ added column
        special_requests: specialRequests.trim() || null,          // ✅ added column
        proposed_move_in_date: proposedMoveIn,                     // ✅ added alias column
        lease_duration_requested: parseInt(leaseDuration),         // ✅ added alias column
        tenant_notes: coverLetter.trim() || null,                  // ✅ added alias column
      }
      ...
      // Update viewing status to application_submitted
      if (viewing?.id) {
        ...
        if (updateError) {
          console.warn("Could not update viewing status:", updateError.message)
        }
      }
        ...
        <Card>
          <CardContent className="pt-8 pb-8 text-center space-y-4">
            ...
            <p className="text-gray-600 max-w-sm mx-auto">
              Your application for <strong>{property?.title}</strong> has been sent to
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2 flex-1">
          <div className="h-8 w-8 rounded-full bg-gray-200 text-gray-400 flex items-center justify-center text-sm font-bold shrink-0">
            3
          </div>
          <div>
      ...
      {viewing && (
        <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-lg p-3">
          ...
          <p className="text-sm text-green-800">
            <strong>
              Viewing{" "}
              {viewing.status === "completed" ? "completed" : "confirmed"}
            </strong>{" "}
            for{" "}
            {new Date(
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Complete your application for {property.title}. The landlord will review
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  >
                    ...
                    <SelectContent>
                      {[3, 6, 12, 18, 24, 36].map((m) => {
                        const belowMin = m < (property.minimum_lease_months || 0)
                        return (
                          <SelectItem key={m} value={String(m)} disabled={belowMin}>
                            {m} months{belowMin ? " (below minimum)" : ""}
                          </SelectItem>
                        )
                      })}
                    </SelectContent>
                  ...
                  <SelectContent>
                    {[0, 1, 2, 3, 4, 5].map((n) => (
                      <SelectItem key={n} value={String(n)}>
                        {n === 0
                          ? "Just me"
                          : `${n} additional ${n === 1 ? "person" : "people"}`}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  id="cover-letter"
                  placeholder="Tell the landlord about yourself, your occupation, why you want this property..."
                  value={coverLetter}
            ...
            <div className="bg-gray-50 border rounded-lg p-4 space-y-2 text-sm">
              ...
              <div className="flex justify-between font-bold text-gray-900 border-t pt-2">
                ...
                <span>
                  R{(
                    property.rent_amount + (property.deposit_amount || 0)
                  ).toLocaleString()}
                </span>
              </div>
              <p className="text-xs text-gray-500">
