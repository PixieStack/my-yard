...
// ✅ FIXED: Added "application_submitted" as a valid status to allow applying
    ...
    try {
      const [propertyResult, viewingResult, applicationResult] = await Promise.all([
        ...
        supabase
          ...
          // ✅ FIXED: Get the latest viewing request by sorting
      ...
      // ✅ FIXED: Only block if no viewing OR viewing status not approvable
      // application_submitted means they already applied — handled by existingApplication check
      if (!view || !canApply(view.status)) {
        // Don't block if application_submitted — existingApplication handles that screen
        if (view?.status !== "application_submitted") {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const insertPayload = {
        ...
        status: "pending",
        preferred_move_in_date: proposedMoveIn,
        ...
        tenant_notes: coverLetter.trim() || null,
      }
