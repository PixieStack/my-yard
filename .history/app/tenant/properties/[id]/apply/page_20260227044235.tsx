...
// Statuses that allow the tenant to apply
...
function getBlockedMessage(viewing: ViewingRequest | null): string {
  ...
  switch (viewing.status) {
    case "pending":
    case "requested":
      return "Your viewing request is awaiting landlord confirmation. You can apply once the landlord confirms your viewing."
    case "scheduled":
...
export default function ApplyForPropertyPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Gate: must have a confirmed/completed viewing to apply
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Guard: validate lease duration against minimum
  ...
  // ─── Already applied ──────────────────────────────────────────────────────
  // Priority 1: Show this before anything else if they already applied
  ...
  // ─── Blocked — no viewing or viewing not in approvable state ──────────────
  // Priority 2
