...
function getBlockedMessage(viewing: ViewingRequest | null): string {
  ...
  switch (viewing.status) {
    ...
    case "scheduled":
      return "Your viewing has been scheduled. You can apply once the landlord marks it as confirmed."
    case "declined":
...
export default function ApplyForPropertyPage() {
  ...
  // Form state — matching ACTUAL DB column names
    ...
    try {
      ...
      const { data: newApp, error: insertError } = await supabase
        ...
        .insert({
          ...
          // Primary columns (original DB columns)
          preferred_move_in_date: proposedMoveIn,               // ✅ actual column
          lease_duration_months: parseInt(leaseDuration),       // ✅ actual column
          cover_letter: coverLetter.trim() || null,             // ✅ actual column

          // Added columns (from our migration)
          additional_occupants: parseInt(additionalOccupants),  // ✅ added column
          additional_occupants_details: occupantDetails.trim() || null, // ✅ added
          special_requests: specialRequests.trim() || null,     // ✅ added

          // Also populate the alias columns so both work
          proposed_move_in_date: proposedMoveIn,                // ✅ added column
          lease_duration_requested: parseInt(leaseDuration),    // ✅ added column
          tenant_notes: coverLetter.trim() || null,             // ✅ added column
        })
        .select("id")
        .single()
      ...
      {viewing && (
        <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-lg p-3">
          ...
          <p className="text-sm text-green-800">
            <strong>
              Viewing {viewing.status === "completed" ? "completed" : "confirmed"}
            </strong>{" "}
            for{" "}
            {new Date(viewing.requested_date + "T00:00:00").toLocaleDateString("en-ZA", {
              day: "numeric",
              month: "long",
              year: "numeric",
            })}{" "}
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="cover-letter">Message to Landlord (optional)</Label>
                <Textarea
                  id="cover-letter"
