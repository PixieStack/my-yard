...
interface ExistingViewing {
  ...
  landlord_message: string | null  // ← correct column name
}
...
function isViewingApproved(status: ViewingStatus): boolean {
  return status === "confirmed" || status === "completed" || status === "application_submitted"
}

function isViewingRejected(status: ViewingStatus): boolean {
  return status === "declined" || status === "cancelled"
}

function isViewingPending(status: ViewingStatus): boolean {
    ...
    try {
      const [propertyResult, viewingResult] = await Promise.all([
        ...
        supabase
          ...
          .select("id, status, requested_date, requested_time, landlord_message")
          .eq("property_id", id)
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-medium text-sm">
                {new Date(existingViewing.requested_date + "T00:00:00").toLocaleDateString("en-ZA", {
                  weekday: "long",
                  day: "numeric",
                  month: "long",
                  year: "numeric",
            ...
            {/* Landlord Message */}
            {existingViewing.landlord_message && (
              <div className="bg-gray-50 border rounded-lg p-3">
                <p className="text-xs text-gray-500 mb-1">Landlord Message</p>
                <p className="text-sm text-gray-700">{existingViewing.landlord_message}</p>
              </div>
            )}

            {/* ── Approved: can apply ────────────────────────────────────── */}
            {isViewingApproved(existingViewing.status) && (
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <p className="font-semibold text-green-900">
                        {existingViewing.status === "application_submitted"
                          ? "Application Already Submitted"
                          : existingViewing.status === "completed"
                      ...
                      <p className="text-green-700 text-sm mt-1">
                        {existingViewing.status === "application_submitted"
                          ...
                          : "Your viewing has been confirmed. You are now eligible to submit a full rental application."}
                      </p>
                    </div>
                  </div>
                </div>
            ...
            {isViewingRejected(existingViewing.status) && (
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className="font-semibold text-red-900">
                      Viewing{" "}
                      {existingViewing.status === "cancelled" ? "Cancelled" : "Declined"}
                    </p>
                    <p className="text-red-700 text-sm mt-1">
                      {existingViewing.status === "cancelled"
                        ? "This viewing was cancelled. You may browse other available properties."
                        : "The landlord has declined this viewing request. You may browse other available properties."}
                    </p>
            ...
            {isViewingPending(existingViewing.status) && (
        ...
        <Card>
          <CardContent className="pt-8 pb-8 text-center space-y-4">
            ...
            <p className="text-gray-600 max-w-sm mx-auto">
              Your request to view <strong>{property.title}</strong> has been sent to the
              landlord. You will be notified once they confirm.
            </p>
