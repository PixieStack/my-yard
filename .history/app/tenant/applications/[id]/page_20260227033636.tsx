...
function Img({ src, alt, className = "" }: { src: string; alt: string; className?: string }) {
  ...
  return (
    <img
      ...
      onError={() => { console.warn("❌ Image failed:", src.substring(0, 80)); setErrored(true) }}
...
export default function PropertyDetailPage() {
  ...
  useEffect(() => {
    if (id) {
      fetchProperty()
      if (profile?.id) { checkFavorite(); checkApplication() }
    }
  }, [id, profile?.id])
  ...
  const fetchProperty = async () => {
    ...
    try {
      ...
      console.log(`✅ Property loaded: ${data.title}, ${sortedImages.length} images`)
  ...
  if (notFound || !property) {
    return (
      <div className="text-center py-12">
        ...
        <Link href="/tenant/properties"><Button>Back to Properties</Button></Link>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      {allImages.length > 0 ? (
        <div className="space-y-3">
          <div className="relative h-80 md:h-[420px] rounded-xl overflow-hidden bg-gray-100">
            {currentImage ? (
              <Img src={currentImage.image_url} alt={`${property.title} ${selectedIndex + 1}`} />
            ) : (
            ...
            {allImages.length > 1 && (
              <>
                <button
                  ...
                  className="absolute left-3 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white rounded-full h-9 w-9 flex items-center justify-center disabled:opacity-30 text-xl transition"
                >‹</button>
                <button
                  onClick={() => setSelectedIndex((i) => Math.min(allImages.length - 1, i + 1))}
                  disabled={selectedIndex === allImages.length - 1}
                  className="absolute right-3 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white rounded-full h-9 w-9 flex items-center justify-center disabled:opacity-30 text-xl transition"
                >›</button>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  { icon: Bed, value: property.bedrooms, label: `Bedroom${property.bedrooms !== 1 ? "s" : ""}` },
                  { icon: Bath, value: property.bathrooms, label: `Bathroom${property.bathrooms !== 1 ? "s" : ""}` },
                  { icon: Building, value: getTypeLabel(property.property_type), label: "Type" },
                  {
                    icon: Calendar,
                    value: property.available_from
                      ? new Date(property.available_from).toLocaleDateString("en-ZA", { day: "numeric", month: "short", year: "numeric" })
                      : "Now",
                    label: "Available"
                  },
          ...
          <Card>
            <CardHeader><CardTitle>What's Included in Rent</CardTitle></CardHeader>
            <CardContent>
