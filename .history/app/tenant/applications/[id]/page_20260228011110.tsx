...
export default function TenantApplicationDetailsPage() {
  ...
  const handleContinueApplication = async () => {
    ...
    try {
      ...
      alert("Failed to continue application. Please try again.")
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {application.viewing_request?.status === "completed" && application.status !== "approved" && application.status !== "rejected" && (
        <Card className="border-amber-200 bg-amber-50">
          <CardContent className="pt-6">
            <div className="flex items-start space-x-3">
              <Eye className="h-6 w-6 text-amber-600 shrink-0 mt-0.5" />
              <div className="w-full">
                <h3 className="font-semibold text-amber-900">Viewing Completed</h3>
                <p className="text-amber-700 text-sm mt-1 mb-4">
                  Your viewing has been completed. Would you like to continue with this application or cancel?
                </p>
