...
export default function TenantApplicationDetailsPage() {
  ...
  const fetchApplicationDetails = async () => {
    try {
      ...
      // Step 4: Fetch viewing request — may not exist, that is fine
      let viewingData = null
      const { data: vd, error: viewingError } = await supabase
        .from("viewing_requests")
        .select("id, requested_date, requested_time, status, tenant_message, landlord_response")
        .eq("property_id", applicationData.property_id)
        .eq("tenant_id", profile!.id)
        .maybeSingle()

      if (viewingError && viewingError.code !== "PGRST116") {
        console.warn("Viewing request fetch issue:", viewingError)
      } else {
        viewingData = vd
      }
            ...
            <CardContent className="space-y-4">
              ...
              {application.lease_duration_requested && (
                <div>
                  ...
                  <p className="font-medium text-sm">{application.lease_duration_requested} months</p>
                </div>
              )}
