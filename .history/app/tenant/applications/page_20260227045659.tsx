...
export default function TenantApplicationsPage() {
  ...
  const fetchApplications = useCallback(async () => {
    ...
    try {
      ...
      // ── Step 1: Fetch raw applications only (no joins to avoid RLS issues) ──
      const { data: appsRaw, error: appsError } = await supabase
        .from("applications")
        .select(`
          id, status, applied_at, proposed_move_in_date,
          lease_duration_requested, tenant_notes, rejection_reason,
          property_id
        `)
      ...
      if (appsError) {
        ...
        setPageError(`Failed to load applications: ${appsError.message}`)
        return
      }
      ...
      const propertyIds = [...new Set(appsRaw.map((a) => a.property_id).filter(Boolean))]

      // ── Step 3: Fetch properties separately ──────────────────────────────
      ...
      if (propertiesError) {
        ...
        // Don't block — show applications without property details
      }

      // ── Step 4: Fetch property images separately ──────────────────────────
      ...
      if (landlordIds.length > 0) {
        ...
        if (landlordsError) {
          console.error("Landlords fetch error:", {
            message: landlordsError.message,
            code: landlordsError.code,
          })
        }
  ...
  const getStatusText = (status: ApplicationStatus) => {
    switch (status) {
      ...
      case "awaiting_landlord_decision": return "Awaiting Decision"
        ...
        return (
          <div className="mt-3 p-3 bg-purple-50 rounded-lg border border-purple-100">
            <p className="text-sm text-purple-800">
              <strong>Viewing Scheduled</strong> — Check your application details for the
        ...
        return (
          <div className="mt-3 p-3 bg-blue-50 rounded-lg border border-blue-100">
            <p className="text-sm text-blue-800">
              <strong>Viewing Requested</strong> — Waiting for the landlord to confirm your
              viewing appointment.
            </p>
          </div>
        )
        ...
        return (
          <div className="mt-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
            <p className="text-sm text-gray-700">
              <strong>Viewing Complete</strong> — The landlord is reviewing your application
