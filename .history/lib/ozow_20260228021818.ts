...
export class OzowPaymentService {
  ...
  /**
   * Generic payment URL builder
   */
  buildPaymentUrl(data: OzowPaymentRequest, hash: string): string {
    const queryParams = new URLSearchParams({
      ...
      IsTest: data.isTest.toString(),
      HashCheck: hash,
    })

    return `${this.config.apiUrl}?${queryParams.toString()}`
  }
