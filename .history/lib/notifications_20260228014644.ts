...
export async function createNotification({ userId, title, message, type, actionUrl }: CreateNotificationParams) {
  try {
    ...
    console.error("Error creating notification:", error instanceof Error ? error.message : JSON.stringify(error))
    throw error
  }
}
...
export async function notifyViewingConfirmed(
  ...
  return createNotification({
    ...
    message: `Your viewing for ${propertyTitle} has been confirmed by the landlord.`,
    type: "viewing",
    actionUrl: `/tenant/viewings/${viewingId}`,
  })
}
...
export async function notifyViewingCompleted(
  ...
  return createNotification({
    ...
    message: `Your viewing for ${propertyTitle} is complete. Ready to submit your application?`,
    type: "viewing",
    actionUrl: `/tenant/applications/new?property=${viewingId}`,
  })
}
