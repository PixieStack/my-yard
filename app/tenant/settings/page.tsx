...
export default function TenantSettingsPage() {
  ...
  const fetchTenantProfile = async () => {
    try {
      const { data, error } = await supabase.from("tenant_profiles").select("*").eq("id", profile?.id).single()
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      const { error: tenantError } = await supabase.from("tenant_profiles").upsert(tenantProfile)
