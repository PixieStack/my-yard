...
export async function POST(request: NextRequest) {
  try {
    ...
    const { error: updateError, data: payment } = await supabase
      .from("payments")
      .update({
        status: dbStatus,
    ...
    // If payment complete, update lease status and create notifications
    if (dbStatus === "complete" && leaseId) {
      if (paymentType === "move_in") {
        ...
        const { data: lease } = await supabase.from("leases").select("property_id, tenant_id, landlord_id").eq("id", leaseId).maybeSingle()
        ...
        if (lease?.tenant_id) {
          await supabase.from("notifications").insert({
            ...
            message: `Your move-in payment of R${Amount} was successful`,
        ...
        if (lease?.landlord_id) {
          await supabase.from("notifications").insert({
            ...
            message: `Tenant move-in payment of R${Amount} received`,
            action_url: `/landlord/payments`,
          })
        }
      } else if (paymentType === "monthly_rent" && payment) {
        // Create notification for tenant
        await supabase.from("notifications").insert({
          user_id: payment.tenant_id,
          type: "payment",
          title: "Rent Payment Successful",
          message: `Your rent payment of R${Amount} was successful`,
        ...
        await supabase.from("notifications").insert({
          ...
          message: `Tenant rent payment of R${Amount} received`,
          action_url: `/landlord/payments`,
        })
      }
    }
...
export async function GET() {
  return NextResponse.json({ message: "Ozow webhook endpoint" })
}
